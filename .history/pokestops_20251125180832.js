...
function createPokestopTooltip(pokestopName, x, y, isRightClick = false) {
    ...
    if (isMobile) {
        // En móvil: posición fija arriba centrado
        ...
        // En PC: posición normal junto al cursor
        tooltip.style.position = 'fixed';
    ...
    // ✅ AUTO-OCULTAR EN MÓVIL DESPUÉS DE 3 SEGUNDOS
...
// Cerrar tooltip al tocar fuera de él en móvil
document.addEventListener('touchstart', function(e) {
    const tooltip = document.getElementById('pokestop-tooltip');
    if (tooltip && tooltip.style.display === 'block') {
        if (!tooltip.contains(e.target) && !e.target.classList.contains('pokestop-icon')) {
            closePokestopTooltip();
        }
    }
}, { passive: true });