...
function exportProfile(profileId) {
    ...
    link.href = url;
    link.download = `profile_${profiles[profileId].name}_${Date.now()}.json`;
...
function importProfile(fileInput) {
    ...
    if (!file) {
        alert(window.i18n ? window.i18n.t('profile.noFileSelected') : 'No file selected');
        return;
    }
    ...
    reader.onload = function(e) {
        try {
            ...
            // Importar un solo perfil
            if (importData.profile) {
                const newProfileId = 'profile_' + Date.now();
                const profileName = importData.profile.name || 'Imported Profile';
            ...
            // Importar múltiples perfiles
                ...
                for (const [oldId, profileData] of Object.entries(importData.profiles)) {
            ...
            alert(window.i18n ? 
                window.i18n.t('profile.importError') : 
                'Error importing profile. Please check the file format.'
            );
        }
    ...
    reader.onerror = function() {
        alert(window.i18n ? 
            window.i18n.t('profile.importReadError') : 
            'Error reading file'
        );
    };
