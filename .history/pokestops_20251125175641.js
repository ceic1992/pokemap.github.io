// ========== MOBILE COOLDOWN OVERRIDE ==========
let MOBILE_COOLDOWN_MODE = window.innerWidth <= 768;

window.addEventListener('resize', function() {
    MOBILE_COOLDOWN_MODE = window.innerWidth <= 768;
});
...
icon.addEventListener('click', function(e) {
    // ========== MODO MÓVIL: ACTIVAR COOLDOWN DIRECTAMENTE ==========
    if (MOBILE_COOLDOWN_MODE) {
        e.preventDefault();
        e.stopPropagation();
        ...
        showMobileCooldownNotification('PokéStop', pokestop.name);
        
        // Actualizar visualmente
        displayPokestops();
        
        console.log('✅ [MÓVIL] Cooldown activado para PokéStop:', pokestop.name);
        return false;
    }
    
    // ========== MODO DESKTOP: ABRIR PREVIEW ==========
    if (isPreviewOpen || previewClickCooldown) {
        console.log('⚠️ Preview already open or in cooldown');
        return;
    }
    ...
    if (pokestop.images && pokestop.images.length > 0) {
        openPreview(pokestop.images, pokestop.name);
    } else {
        console.log('No images for this PokéStop');
        isPreviewOpen = false;
    }
});
...
// ========== DESHABILITAR PREVIEW EN MÓVIL ==========
if (window.innerWidth <= 768) {
    console.log('📱 Preview de PokéStops deshabilitado en móvil');
    
    // Sobrescribir la función openPreview para que no haga nada en móvil
    const originalOpenPreview = window.openPreview;
    window.openPreview = function() {
        console.log('📱 Preview bloqueado en móvil');
        return false;
    };
}
