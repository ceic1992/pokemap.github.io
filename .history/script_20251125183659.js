...
function displayBossIcons() {
    ...
    Object.entries(bosses).forEach(([bossName, bossData]) => {
        ...
        if (location && location.map_pos) {
            ...
            // ✅ ENCODEAR EL NOMBRE PARA MANEJAR ESPACIOS Y CARACTERES ESPECIALES
            const encodedBossName = encodeURIComponent(bossName);
            img.src = `resources/bosses/${encodedBossName}.webp`;
            img.alt = bossName;
            
            img.onerror = function() {
                console.warn(`Boss image not found: ${bossName}.webp, trying default`);
                this.src = 'resources/bosses/default-boss.webp';
                this.onerror = function() {
                    console.error(`Default boss image also not found!`);
            ...
            icon.addEventListener('click', function(e) {
                e.stopPropagation();
                if (!isBossAvailable(bossName)) {
                    if (typeof showBossInfo === 'function') {
    ...
    console.log(`✅ Displayed ${bossIcons.length} boss icons (filter: ${currentRegionFilter})`);
}
